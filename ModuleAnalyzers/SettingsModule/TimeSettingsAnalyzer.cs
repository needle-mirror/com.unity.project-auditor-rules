...
namespace Unity.ProjectAuditorRules.SettingsModuleAnalyzers
{
    class TimeSettingsAnalyzer : SettingsModuleAnalyzer
    {
        ...
        const float k_DefaultValueTolerance = 1e-7f;
        ...
        {
            // Only report when the setting still equals its default value
            if (Mathf.Abs(UnityEngine.Time.fixedDeltaTime - 0.02f) < k_DefaultValueTolerance)
            {
                yield return context.CreateIssue(IssueCategory.ProjectSetting, k_FixedTimestepDescriptor.Id)
                    .WithLocation("Project/Time");
            }
            if (Mathf.Abs(UnityEngine.Time.maximumDeltaTime - 0.1f) < k_DefaultValueTolerance)
            {
