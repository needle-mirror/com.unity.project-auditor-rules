...
#pragma warning restore CS0649
        ...
        ReportItem AnalyzeStreamingAssets(AnalysisContext context)
        {
            ...
            {
                // Skip Unity .meta files: they live alongside the real assets but are not packaged into the player
            ...
            // Multiply in 64-bit using long.
            var folderSizeLimitBytes = (long)folderSizeLimitMB * 1024 * 1024;

            if (totalBytes <= folderSizeLimitBytes)
                return null;

            return context.CreateIssue(IssueCategory.ProjectSetting, k_StreamingAssetsFolderDescriptor.Id, Formatting.FormatSize((ulong)totalBytes))
                .WithLocation("Assets/StreamingAssets");
        }
    }
}
