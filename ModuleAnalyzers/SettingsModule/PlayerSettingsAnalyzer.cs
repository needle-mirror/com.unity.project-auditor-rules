...
namespace Unity.ProjectAuditorRules.SettingsModuleAnalyzers
{
    class PlayerSettingsAnalyzer : SettingsModuleAnalyzer
    {
            ...
            "Enable <b>Lightmap Streaming</b> in <b>PProject Settings > Player > Other Settings > Rendering</b>.")
        {
            ...
            {
                var buildTargetGroup = BuildPipeline.GetBuildTargetGroup(analysisParams.Platform);
                SetLightmapStreaming(buildTargetGroup, true);
                return true;
            },
        };
        ...
        public override IEnumerable<ReportItem> Analyze(SettingsAnalysisContext context)
        {
            ...
            if (!IsLightmapStreamingEnabled(buildTargetGroup))
            {
        ...
        internal static bool IsSplashScreenEnabledAndCanBeDisabled()
        {
            if (!PlayerSettings.SplashScreen.show)
                return false;
            var type = typeof(Editor).Assembly.GetType("UnityEditor.PlayerSettingsSplashScreenEditor");
            if (type == null)
                return false;
        ...
        internal static bool CheckIL2CPPCompilerConfiguration(Il2CppCompilerConfiguration compilerConfiguration, AnalysisParams analysisParams)
        {
            ...
            return PlayerSettings.GetIl2CppCompilerConfiguration(namedBuildTarget) == compilerConfiguration;
        }
        ...
        {
            MethodInfo method = typeof(PlayerSettings).GetMethod("GetLightmapStreamingEnabledForPlatformGroup", BindingFlags.NonPublic | BindingFlags.Static);
            if (method == null)
                throw new NotSupportedException("Getting Lightmap Streaming per platform is not supported");

            return (bool)(method.Invoke(null, new object[1] { platform }) ?? throw new NotSupportedException("Getting Lightmap Streaming per platform is not supported"));
        }
    }
}
